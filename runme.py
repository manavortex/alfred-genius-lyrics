...
def thumbs():
    ...
    return _thumbs
def database():
    """Return global `Db` object, creating first if necessary."""
    global _db
    if _db is None:
        _db = Db(wf.datafile(DB_NAME))
    return _db

class AlfredGeniusLyrics(object):
    """Workflow controller."""
...
if __name__ == '__main__':
    ...
    lyr = AlfredGeniusLyrics()
    sys.exit(lyr.run(lyr.wf.run))
